...
class DataLoader:
        ...
        with self.driver.session() as session: 
            ...
            for index, row in trips.iterrows(): 
                ...
                if len(batch_data) >= batch_size: 
                    session.execute_write(self.create_trip_batch, batch_data)
                    batch_data = []
    ...
    @staticmethod
    def create_trip_batch(tx, batch_data): 
        query = """
            UNWIND $batch AS row
            MERGE (pickup:Location {name: row.PULocationID})
            MERGE (dropoff:Location {name: row.DOLocationID})
            MERGE (pickup)-[:TRIP {
                distance: row.trip_distance, 
                fare: row.fare_amount, 
                pickup_dt: row.pickup_datetime,
                dropoff_dt: row.dropoff_datetime
            }]->(dropoff)
        """
